...
const DEMO_PRODUCTS = [
  { id:'d1', handle:'ivory-wrap-dress', title:'Ivory Wrap Dress',
    niche:'Corporate Luxe', price:'148.00', tag:'Bestseller',
    fabric:'silk', colors:['#F0EDE6','#DDD5C4','#C4B49A'],
    desc:'Effortlessly transitions from boardroom to dinner',
    variantId:'demo-1', image:'' },
  { id:'d2', handle:'linen-coastal-maxi', title:'Linen Coastal Maxi',
    niche:'Coastal Grandmother', price:'168.00', tag:'New Arrival',
    fabric:'linen', colors:['#D8E8D8','#B8CEB8','#96B096'],
    desc:'Relaxed luxury for the effortlessly chic',
    variantId:'demo-2', image:'' },
  { id:'d3', handle:'satin-bridal-slip', title:'Satin Bridal Slip',
    niche:'Minimalist Bride', price:'298.00', tag:'Limited',
    fabric:'satin', colors:['#F4F2E8','#E8E4D4','#D4CEBC'],
    desc:'Pure bridal elegance in bias-cut satin',
    variantId:'demo-3', image:'' },
  { id:'d4', handle:'resort-kaftan', title:'Resort Kaftan',
    niche:'Resort Luxury', price:'188.00', tag:'Summer Edit',
    fabric:'chiffon', colors:['#E8F2E8','#C8DEC8','#A8CAA8'],
    desc:'Amalfi Coast energy wherever you are',
    variantId:'demo-4', image:'' },
  { id:'d5', handle:'power-sheath', title:'Power Sheath',
    niche:'Corporate Luxe', price:'178.00', tag:'Classic',
    fabric:'structured', colors:['#1C2E20','#142218','#243826'],
    desc:'Command the room without saying a word',
    variantId:'demo-5', image:'' },
  { id:'d6', handle:'sage-linen-dress', title:'Sage Linen Dress',
    niche:'Coastal Grandmother', price:'218.00', tag:'Editor Pick',
    fabric:'linen', colors:['#C8D8C4','#A8BCA4','#8CA48C'],
    desc:'Old-money aesthetic meets modern comfort',
    variantId:'demo-6', image:'' },
]
    ...
    <div
      ...
      <div style={{
        ...
        {sp?.images?.edges?.[0]?.node?.url ? (
          ...
          <img
            src={sp.images.edges[0].node.url}
            alt={sp.title}
      ...
      <div style={{padding:'20px 18px 22px'}}>
        ...
        }}>{sp?.title || product.title}</h3>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <span style={{
            fontFamily:'Jost,sans-serif',fontSize:14,
            color:'#5A6B60',letterSpacing:'0.04em',
          }}>
            ${parseFloat(sp?.priceRange?.minVariantPrice?.amount || product.price).toFixed(2)}
          </span>
...
export default function Home() {
  ...
  useEffect(()=>{
    getAllProducts().then(prods => {
      if(prods?.length) setShopifyProducts(prods)
    }).catch(()=>{})
  },[])
  ...
  const filtered=activeNiche==='All'
    ? DEMO_PRODUCTS
    : DEMO_PRODUCTS.filter(p=>p.niche===activeNiche)

  const matchShopify=(product:typeof DEMO_PRODUCTS[0])=>
    shopifyProducts.find(sp=>
      sp.handle===product.handle ||
